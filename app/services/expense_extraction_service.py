...
def _parse_json_from_model(raw_text: str) -> dict[str, Any]:
    ...
    if cleaned.startswith("{") and cleaned.endswith("}"):
        try:
            ...
            # Fall through to regex search if direct parsing fails
            pass

    match = _JSON_OBJECT_PATTERN.search(cleaned)
    if not match:
        logger.error("Could not find a JSON object in model output. Raw: %s", raw_text)
    ...
    try:
        ...
        logger.error(
            "Failed to parse extracted JSON from model output. Raw: %s",
            raw_text,
            exc_info=True,
        )
